...
def get_pop_cat():
    ...
    for val in ages:
        cat_list.append(get_state_pop().loc[(get_state_pop()['AGE']>=
            val)&(get_state_pop()['AGE']<val+100)].groupby('state').sum().reset_index()[['state', 'POPEST2019_CIV'
            ]].rename(columns = {'POPEST2019_CIV': str(val) + 'PlusPop'}))
...
def get_vacc():
    ...
    vacc_state_totals = vacc.groupby(['date','state'])[['Series_Complete_Yes', 'Series_Complete_12Plus', 'Series_Complete_18Plus', 
...
def get_all_data():
    ...
    all_data = reduce(lambda  left,right: pd.merge(left,right,
                                        on=['state', 'date'], how='outer'), [get_cases(), get_hosp(), get_test(), 
                                                                             get_vacc_pop_pct()])
